...
function SecurityManager(options) {
    ...
    this.isBanned = function(ip) {
        ...
        return {
            remainingTime: ban.permanent ? Infinity : Math.floor((ban.bannedUntil - now) / 1000),
            reason: ban.reason,
    ...
    this.recordConnection = function(ip) {
        ...
        if (!record || (now - record.firstConnection) > _this.options.rateLimit.window) {
    ...
    this.banIP = function(ip, reason, duration, permanent) {
        ...
        var now = Date.now();
        var bannedUntil = permanent ? now + _this.options.ban.permanentDuration : now + duration;
    ...
    this.cleanup = function() {
        ...
        for (var ip in _this.connectionCounts) {
            if ((now - _this.connectionCounts[ip].firstConnection) > _this.options.rateLimit.window * 2) {
        ...
        for (var ip in _this.malformedCounts) {
            if ((now - _this.malformedCounts[ip].firstMessage) > _this.options.rateLimit.window * 2) {
        ...
        for (var ip in _this.floodCounts) {
            if ((now - _this.floodCounts[ip].firstFlood) > _this.options.rateLimit.window * 2) {
