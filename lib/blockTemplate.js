...
var BlockTemplate = (module.exports = function BlockTemplate(
  ...
  /**
   * Generates an array of versions for multi_version (overt AsicBoost) support.
   * @param {number} count - Number of versions to generate
   * @param {string} mode - Generation mode: 'sequential' or 'mask-based'
  ...
  this.generateVersionArray = function (count, mode) {
    ...
    if (mode === 'sequential') {
      // Generate sequential versions: base, base+1, base+2, etc.
      for (var i = 0; i < count; i++) {
        versions.push(baseVersion + i);
      }
    } else {
      // Fallback to sequential if mode not recognized
      for (var i = 0; i < count; i++) {
        versions.push(baseVersion + i);
      }
    }
