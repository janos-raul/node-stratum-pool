...
var pool = module.exports = function pool(options, authorizeFn){
	...
	securityManager.on('ipBanned', function(ip, reason, duration, permanent) {
		if (permanent) {
			emitWarningLog('PERMANENTLY BANNED IP ' + ip + ' for ' + duration + 's - Reason: ' + reason + ' [' + options.coin.name + ']');
		} else {
			emitWarningLog('Banned IP ' + ip + ' for ' + duration + 's - Reason: ' + reason + ' [' + options.coin.name + ']');
		}
	...
	securityManager.on('strikeAdded', function(ip, reason, strikeCount) {
		emitWarningLog('Strike #' + strikeCount + ' for IP ' + ip + ': ' + reason + ' [' + options.coin.name + ']');
	});

	securityManager.on('rateLimitExceeded', function(ip, type, count) {
		emitWarningLog('Rate limit exceeded for IP ' + ip + ' (' + type + ': ' + count + ') [' + options.coin.name + ']');
	});

	securityManager.on('malformedMessage', function(ip, message, count) {
		if (count === 1) {
			emitWarningLog('Malformed message from IP ' + ip + ' [' + options.coin.name + ']');
		} else {
			emitWarningLog('Malformed message #' + count + ' from IP ' + ip + ' [' + options.coin.name + ']');
		}
	});

	securityManager.on('socketFlooded', function(ip, count) {
		emitWarningLog('Socket flooding detected from IP ' + ip + ' (count: ' + count + ') [' + options.coin.name + ']');
	});
		...
		var generateProgress = function(){
			...
			_this.daemon.cmd(cmd, [], function(results) {
				...
				if (!results || !results.length || !results[0].response) {
					emitWarningLog('Unable to get sync progress - no response from daemon');
					return;
				}
				...
				_this.daemon.cmd('getpeerinfo', [], function(results){
					if (!results || !results[0] || !results[0].response || !results[0].response.length) {
						emitWarningLog('No peers connected - unable to determine sync progress');
						return;
					}
					...
					if (peersWithHeight.length === 0) {
						emitWarningLog('Connected to ' + peers.length + ' peers but none have reported block height');
						return;
					}
    ...
    function SetupPeer(){
        ...
        _this.peer.on('connected', function() {
            emitLog('Connection successful', 'P2P');
        ...
        function (results) {
            for (var i = 0; i < results.length; i++) {
               ...
               // console.log('  instance:', result.instance.index);
                ...
                else if (result.response && result.response !== null && result.response !== '') {
                ...
                SubmitBlock(blockHex, function(){
                    CheckBlockAccepted(shareData.blockHash, function(isAccepted, tx){
                        ...
                        GetBlockTemplate(function(error, result, foundNewBlock){
                            if (foundNewBlock)
								emitSpecialLog('Block notification via RPC after block submission', 'Block');
                        });
...
_this.stratumServer.on('started', function(){
   ...
   // console.log('getJobParams exists:', !!(_this.jobManager.currentJob && _this.jobManager.currentJob.getJobParams));
    ...
    if (_this.jobManager.currentJob && typeof _this.jobManager.currentJob.getJobParams === 'function') {
        ...
        emitErrorLog('Cannot broadcast - currentJob or getJobParams not available', 'Stratum');
    }
    finishedCallback();


    }).on('broadcastTimeout', function(){
        emitLog('No new blocks for ' + (options.jobRebroadcastTimeout || 55) + ' seconds - updating transactions & rebroadcasting work', 'JobManager');
        ...
        client.on('asicboostEnabled', function(capabilities) {
			emitLog('AsicBoost enabled for ' + client.getLabel() + ', mask: 0x' + capabilities.versionMask.toString(16), 'AsicBoost');
        ...
        client.on('difficultyChanged', function(diff){
            ...
            // Send job - client will handle AsicBoost-specific modifications in sendMiningJob
    ...
    if (client.asicboost && params.version && _this.jobManager.currentJob) {
        try {
    ...
    var result = _this.jobManager.processShare(
		...
		if (result.error) {
			...
			emitWarningLog('Invalid share from ' + params.name + ': ' + result.error[1], 'Share');
		} else {
			emitLog('Valid share from ' + params.name + (isAsicBoost ? ' (ASICBoost)' : ''), 'Share');
		}
    ...
    resultCallback(result.error, result.result ? true : null);
			...
			var shouldBan = securityManager.recordMalformed(client.remoteAddress, message);
			if (shouldBan) {
			...
			emitWarningLog('Rejected incoming connection from ' + client.remoteAddress + ' banned for ' + remainingBanTime + ' more seconds');
		}).on('forgaveBannedIP', function(){
			emitLog('Forgave banned IP ' + client.remoteAddress);
		}).on('unknownStratumMethod', function(fullMessage) {
			emitWarningLog('Unknown stratum method from ' + client.getLabel() + ': ' + fullMessage.method);
         ...
         //   console.log('  results.length:', results.length);
          ...
          //  console.log('[DEBUG] validResults.length:', validResults.length);
            ...
            if (validResults.length >= 1){
                ...
                emitErrorLog('Block NOT FOUND in blockchain - likely rejected - ' + blockHash, 'Block');
                callback(false);
            }
        }
    );
