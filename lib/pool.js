        ...
        _this.daemon.cmd('getblocktemplate',
            ...
            function(result){
                if (result.error){
                  ...
                  //  console.log('DEBUG: getblocktemplate raw response received');
                  //  console.log('DEBUG: RPC response version field: ' + result.response.version);
                  //  console.log('DEBUG: RPC response version (hex): 0x' + result.response.version.toString(16));
                  //  console.log('DEBUG: RPC response previousblockhash: ' + result.response.previousblockhash);
                  //  console.log('DEBUG: RPC response height: ' + result.response.height);
    ...
    if (typeof (_this.jobManager.currentJob) !== 'undefined' && blockHash !== _this.jobManager.currentJob.rpcData.previousblockhash) {
        GetBlockTemplate(function (error, result) {
            if (error) {
                ...
                // MISSING: Process the new template and broadcast jobs
                var processedNewBlock = _this.jobManager.processTemplate(result);
                if (processedNewBlock) {
                    emitLog('New block processed from notification, broadcasting jobs');
                }
            }
        })
    }
