            ...
            for (var i = 0; i < results.length; i++) {
               ...
               // console.log('  instance:', result.instance.index);
                ...
                // ADD: Check for other rejection responses
                    ...
                    console.log('[ERROR] Block rejected by daemon: ' + result.response);
                ...
                SubmitBlock(blockHex, function(){
                    CheckBlockAccepted(shareData.blockHash, function(isAccepted, tx){
                        ...
                        GetBlockTemplate(function(error, result, foundNewBlock){
                            if (foundNewBlock)
                                console.log('Block notification via RPC after block submission');
                        });
...
_this.stratumServer.on('started', function(){
   ...
   // console.log('getJobParams exists:', !!(_this.jobManager.currentJob && _this.jobManager.currentJob.getJobParams));
    ...
    if (_this.jobManager.currentJob && typeof _this.jobManager.currentJob.getJobParams === 'function') {
        ...
        console.log('Cannot broadcast - currentJob or getJobParams not available');
    }
    finishedCallback();


    }).on('broadcastTimeout', function(){
        console.log('No new blocks for ' + (options.jobRebroadcastTimeout || 55) + ' seconds - updating transactions & rebroadcasting work');
        ...
        client.on('asicboostEnabled', function(capabilities) {
            console.log('AsicBoost enabled for ' + client.getLabel() + 
        ...
        client.on('asicboostDisabled', function() {
            console.log('Traditional mining mode for ' + client.getLabel());
        });
    ...
    if (client.asicboost && params.version && _this.jobManager.currentJob) {
        try {
           ...
           // console.log('  Detected as ASICBoost:', isAsicBoost);
            ...
            console.log('[Error] ASICBoost detection failed:', e.message);
            isAsicBoost = false;
        }
    }
    ...
    resultCallback(result.error, result.result ? true : null);
            ...
            console.log('Forgaved banned IP ' + client.remoteAddress);
...
function CheckBlockAccepted(blockHash, callback){
    console.log('! Checking if block was accepted: ' + blockHash);
         ...
         //   console.log('  results.length:', results.length);
          ...
          //  console.log('[DEBUG] validResults.length:', validResults.length);
            ...
            if (validResults.length >= 1){
                ...
                console.log('! Block NOT FOUND in blockchain - likely rejected');
                callback(false);
            }
        }
    );
