...
var diff1 = BigInt('0x00000000ffff0000000000000000000000000000000000000000000000000000');
...
var JobManager = module.exports = function JobManager(options){
     ...
     */
this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, versionMask, isSoloMining, client, version, isAsicBoost, clientContext) {
...
if (version && isAsicBoost) {
    ...
    if (job.target >= headerBigNum) {
        ...
        if (isAsicBoost) {
			_this.emit('log', 'success', 'ASICBoost block found! Worker: ' + workerName + 
					   ', Version: 0x' + versionInt.toString(16));
        }
    } else {
	...
	if (isSoloMining && !blockHex) {
		_this.emit('log', 'debug', 'Solo share from ' + workerName + ' (diff: ' + shareDiff.toFixed(8) + ') - no reward for non-block shares');
	...
	if (isSoloMining && blockHex) {
	_this.emit('log', 'success', 'SOLO BLOCK found by ' + workerName + 
			   ', Difficulty: ' + shareDiff.toFixed(8) +
	...
	_this.emit('share', {
		...
		isSoloMining: isSoloMining,
		paymentMode: isSoloMining ? 'solo' : ((clientContext && clientContext.paymentMode) || 'default')
	}, blockHex);
