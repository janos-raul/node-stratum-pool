...
if (version && isAsicBoost) {
    ...
    if (job.target >= headerBigNum) {
        ...
        if (isAsicBoost) {
            console.log('[BLOCK] AsicBoost block found! Worker: ' + workerName + 
...
if (isSoloMining && !blockHex) {
    console.log('[Solo] Share from ' + workerName + ' (diff: ' + shareDiff.toFixed(8) + ') - no reward for non-block shares');
...
if (isSoloMining && blockHex) {
    console.log('[SOLO BLOCK] Block found by solo miner: ' + workerName + 
