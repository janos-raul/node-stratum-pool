...
this.isWhatsminer = false;      // Whatsminer detection flag
	...
	function handleMessage(message){
		...
		switch(message.method){
			...
			case 'mining.suggest_difficulty':
				...
				var poolManagedDiff = 125000;
				
				var suggestedDiff = message.params[0];
				_this.emit('log', 'info', 'Miner ' + _this.remoteAddress + ' suggested diff ' + suggestedDiff + 
     ...
     */
    _this.emit('log', 'info', 'MRR reconnect request from ' + _this.getLabel());
    ...
    if (params.length === 0) {
        ...
        _this.emit('log', 'info', 'MRR redirect to ' + newHost + ':' + newPort);
	...
	function handleAuthorize(message, replyToSocket) {
		...
		if (_this.connectionSequence.join(',') === 'configure,subscribe,authorize') {
        _this.emit('log', 'info', 'Complete Whatsminer sequence detected for ' + _this.workerName);
        ...
        if (_this.preAuthAsicBoost && !_this.asicboost) {
            ...
            _this.versionMask = _this.preAuthAsicBoost.versionMask;
            _this.negotiatedExtensions = _this.preAuthAsicBoost.extensions;
            _this.emit('log', 'info', 'Restored ASICBoost state after auth');
        }
    }
		...
		options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, function (result) {
			...
			if (result.disconnect === true) {
				...
				if (_this.authorized) {
					...
					_this.emit('log', 'info', 'Miner authorized: ' + _this.workerName + ', UserAgent: ' + (_this.userAgent || 'unknown'));
				}
			}
		});
	}
	...
	function parseWorkerParameters(passwordString) {
		...
		_this.emit('log', 'debug', 'Parsing auth parameters: ' + passwordArgs.join(','));
		...
		for (var i = 0; i < passwordArgs.length; i++) {
			...
			switch (key) {
				case 'd':
					...
					_this.emit('log', 'debug', 'Set initial difficulty: ' + _this.initialDifficulty);
					break;
				...
				case 'md':
					if (!_this.varDiff) {
					 ...
					 _this.emit('log', 'debug', 'Set minimum difficulty: ' + _this.minimumDifficulty);
					break;
					
				case 'm':
					_this.isSoloMining = value.trim().toLowerCase() === 'solo';  // Keep existing property name
					_this.emit('log', 'info', 'Solo mining mode: ' + _this.isSoloMining + ' for ' + _this.workerName);
					break;
					
				default:
					_this.emit('log', 'warning', 'Unknown auth parameter: ' + key + '=' + value);
					break;
			}
		}
	}
    ...
    function handleConfigure(message){
		...
		_this.emit('log', 'debug', 'mining.configure called for client: ' + _this.getLabel());
		
		 if (!_this.authorized) {
        _this.emit('log', 'info', 'Whatsminer flow detected - configure before authorize');
        _this.isWhatsminer = true;
        
        // Extend socket timeout for Whatsminer
        if (options.socket && options.socket.setTimeout) {
            options.socket.setTimeout(30000); // 30 seconds
            _this.emit('log', 'debug', 'Extended socket timeout for Whatsminer');
        }
    }
        ...
        if (extensions.includes("version-rolling")) {
			...
			var poolVersionMask = options.versionMask ? parseInt(options.versionMask, 16) : 0x3fffe000;
            var clientRequestedMask = extensionParams["version-rolling.mask"];
			var clientMinBitCount = extensionParams["version-rolling.min-bit-count"] || options.versionRollingMinBits || 16;
			...
			if (!_this.authorized) {
				...
				_this.emit('log', 'info', 'Stored pre-auth AsicBoost config for Whatsminer');
			}
        ...
        bitCount: bitCount
				...
				_this.emit('log', 'warning', 'Client ' + (_this.workerName || 'unknown') + 
	...
	this.sendMiningJob = function(jobParams){
        ...
        _this.emit('log', 'debug', 'Sending job to Whatsminer ' + (_this.workerName || 'unauthorized'));
		}
 ...
 */
 var StratumServer = exports.Server = function StratumServer(options, authorizeFn){
	...
	if (options.versionMask) {
		poolVersionMask = parseInt(options.versionMask, 16);
		_this.emit('log', 'info', 'Server configured with versionMask: 0x' + poolVersionMask.toString(16));
	} else {
		poolVersionMask = 0x3fffe000;
		_this.emit('log', 'info', 'No versionMask in options, using default 0x' + poolVersionMask.toString(16));
	}
    ...
    this.handleNewClient = function (socket){
        ...
        client.on('minerAuthorized', function(workerName) {
            _this.emit('log', 'info', 'Miner authorized: ' + workerName + ', UserAgent: ' + (client.userAgent || 'unknown'));
    ...
    this.broadcastMiningJobs = function(jobParams){
        ...
        if (asicboostCount > 0 || traditionalCount > 0) {
			_this.emit('log', 'debug', 'Broadcast job to ' + 
    ...
    (function init(){
        ...
        setInterval(function() {
            ...
            if (stats.totalClients > 0) {
					_this.emit('log', 'info', 'AsicBoost Stats - Total: ' + stats.totalClients + 
        ...
        Object.keys(options.ports).forEach(function(port){
            net.createServer({allowHalfOpen: false}, function(socket) {
                ...
                if (serversStarted == Object.keys(options.ports).length) {
                    _this.emit('log', 'special', 'All servers started with AsicBoost support enabled');
                    _this.emit('started');
                }
            });
        });
