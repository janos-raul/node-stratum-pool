...
var varDiff = (module.exports = function varDiff(port, varDiffOptions) {
  ...
  var bufferSize = Math.max(
    1,
    Math.round((varDiffOptions.retargetTime / varDiffOptions.targetTime) * 4)
  );
  var tMin = varDiffOptions.targetTime - variance;
  var tMax = varDiffOptions.targetTime + variance;
...
this.manageClient = function (client) {
    ...
    client.on("submit", function () {
      ...
      if (avg > tMax && client.difficulty > effectiveMinDiff) {
